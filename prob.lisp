...
(defun hypothesis-prob-unnormalized (i limes cherries)
  (let* ((hi (elt *cherry-per-pack* i))
	 (likelihood (* (expt (- 1 hi) limes)
			(expt hi cherries))))
    (* likelihood (elt *pack-probability* i))))

(defun hypothesis-prob (i limes cherries)
  (/ (hypothesis-prob-unnormalized i limes cherries)
  ...
  (with-open-file (s "/dev/shm/candy.dat" :direction :output
   ...
   (loop for l upto 10 do
	...
	(loop for i below (length *pack-probability*) do
	     (format s "~d " (hypothesis-prob i l 0)))
	(terpri s)))
...
(defun next-candy-is-lime-prob (limes cherries)
  (loop for i below (length *pack-probability*) sum
	 ...
	 (* draw-lime-prob (hypothesis-prob i limes cherries)))))
